...
class Database:
    ...
    def update(self, idx, title="", author="", year="", isbn=""):
        self.cur.execute(
            "UPDATE books SET title=?, author=?, year=?, isbn=? WHERE id=?", (title, author, year, isbn, idx))
        self.conn.commit()

    def delete(self, idx):
        self.cur.execute("DELETE FROM books WHERE id=?", (idx,))
        self.conn.commit()
